...
/// Interface defining the required methods invoked by Observeds.
/// Implemented by [FloopElement].
abstract class ObservedListener {
  // @protected
  // registerObserved(Observed observed);
  // @protected
  // unregisterObserved(Observed observed);

  @protected
  Set<ObservedNotifier> get observeds;
...
class ObservedController {
  ...
  static stopListening() => _activeListener = null;

  static bool get isListening => _activeListener != null;

  static bool _posponeNotifications = false;

  static void posponeNotifications() {
    _posponeNotifications = true;
  }

  static void disablePosponeNotifications() {
    assert(_posponeNotifications);
    _posponeNotifications = false;
  }

  static final Set<ObservedNotifier> posponedObserveds = Set();

  static notifyChangeToListeners(ObservedNotifier observed) {
    if (_posponeNotifications) {
      posponedObserveds.add(observed);
    } else {
      for (var listener in observed.listeners) {
        listener.onObservedChange(observed);
      }
    }
  }

  static unsubscribeListener(ObservedListener listener) {
    for (var observed in listener.observeds) {
      observed.unregisterListener(listener);
    }
    listener.observeds.clear();
    assert(() {
      ObservedController._debugReadListeners.remove(listener);
      return true;
    }());
  }

  static final _debugReadListeners = Set<ObservedListener>();

  static int get length => _debugReadListeners.length;

  static debugReset() {
    for (var listener in _debugReadListeners.toList()) {
      unsubscribeListener(listener);
    }
  }
}

abstract class ObservedNotifier {
  Iterable<ObservedListener> get listeners;
  unregisterListener(ObservedListener listener);
  notifyRead();
  notifyChange();
}
...
mixin ObservedNotifierMixin implements ObservedNotifier {
  // static ObservedListener _currentBuild;
  // static startListening(ObservedListener element) => _currentBuild = element;
  // static stopListening() => _currentBuild = null;
  ...
  @protected
  final Set<ObservedListener> _listeners = Set();
  Iterable<ObservedListener> get listeners => _listeners;
  ...
  void notifyRead() {
    ...
    if (ObservedController.isListening) {
      assert(() {
        ObservedController._debugReadListeners.add(activeListener);
        return true;
      }());
      activeListener.observeds.add(this);
      _listeners.add(activeListener);
    }
  }
  ...
  forgetSubscriptions() {
    ...
    for (var listener in _listeners) {
      listener.observeds.remove(this);
    }
    _listeners.clear();
  }
