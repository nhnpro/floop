...
double transition(
  ...
  var transitionObject = _Transition.of(key);
...
Object transitionEval(
  ...
  if (key != null) {
    transitionObj = _Transition.of(key);
  }
  key ??= _createKey();
...
void _applyToTransitions(
  ...
  if (key != null) {
    apply(_Transition.of(key));
  } else if (context != null) {
    _contextToKeys[context]?.forEach((key) => apply(_Transition.of(key)));
  } else {
...
void _stopAndDispose(Object key) {
  _Transition.of(key)?.stopAndDispose();
}
...
double transitionOf(Object key) {
  return _Transition.of(key)?.currentValue;
}

class _Transition extends Repeater {
  static final Map<Object, _Transition> _keyToTransition = Map();

  factory _Transition.of(Object key) => _keyToTransition[key];
